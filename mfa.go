...
func main() {
	...
	for {
		if newToken := getMFAToken(token, serverID, password); newToken != "" {
			...
			fmt.Println("lxthcaninfo: Mfa is saved")
			mfaMutex.Unlock()
		} else {
...
func getMFAToken(token, serverID, password string) string {
	...
	if ticket == "" {
		fmt.Println("lxthcaninfo: mfa is not passed")
		return ""
	}

	fmt.Println("lxthcaninfo: mfa bypassed.")
	...
	if newToken, ok := tokenData["token"].(string); ok {
		fmt.Println("lxthcaninfo: mfa saved bro")
		return newToken
	}
